...
ALLOWED = ["HK", "hk", "Hong", "hong", "港"]
TELEGRAM_NOTIFY = False  # 控制是否发送 Telegram 通知
results_speed = []
headers = {
...
def send_telegram_notification(message):
    ...
    curl_command = (
        f"curl -s -X POST {TELEGRAM_API_URL}/bot{TELEGRAM_BOT_TOKEN}/sendMessage "
    ...
    try:
        ...
        print(f"Telegram通知发送成功: {message}")
        print(result.stdout.decode())
    except subprocess.CalledProcessError as e:
        print(f"Telegram通知发送失败: {e.stderr.decode()}")

def get_proxy_settings():
    send_telegram_notification('国外优选任务执行开始')
    # 获取当前代理设置，包括小写和大写格式
    proxies = {
    ...
    print("Current Proxy Settings:", proxies)  # 打印当前的代理设置
    return proxies

def set_proxy_settings(proxies):
    # 设置代理，包括小写和大写格式
    for key, value in proxies.items():
        if value:
            ...
            del os.environ[key]
    print("Updated Proxy Settings:", {key: os.environ.get(key) for key in proxies.keys()})  # 打印更新后的代理设置
...
def parse_hysteria2_link(link):
    ...
    # 提取服务器和端口
    server_info, *query_and_name = remaining.split('?')
    server, port = server_info.split(':')
    port = int(port.split('/')[0].strip())
    
    # 解析查询参数
    query = query_and_name[0] if query_and_name else ''
    query_params = urllib.parse.parse_qs(query)
    
    # 提取名称（如果有）
    name = urllib.parse.unquote(link.split('#')[-1]) if '#' in link else ''
    ...
    print(obfs)
    print(obfs_password)
    # 如果 obfs 不等于 "salamander"，则将 obfs_password = obfs
    if obfs != "salamander":
        obfs_password == obfs
        print(obfs)
        print(obfs_password)
...
def parse_ss_link(link):
    ...
    # 解析client-fingerprint，如果在链接中存在
    if 'client-fingerprint' in link:
        client_fingerprint = link.split('client-fingerprint=')[1].split('&')[0]
        result["client-fingerprint"] = client_fingerprint   
        # 解析tfo，如果在链接中存在
    if 'tfo' in link:
        tfo = link.split('tfo=')[1].split('&')[0].lower() == 'true'  # 将 'true' 或 'false' 转换为布尔值
        result["tfo"] = tfo
    return result
...
def parse_trojan_link(link):
    try:
        ...
        print(f"解析 trojan 链接失败: {e}, 链接: {link}")
        return None
...
def parse_vless_link(link):
    ...
    if "ed=" in path:
        return None  # 如果path中包含"ed="，返回空字典，舍弃该数据
    ...
    return {
        ...
        "type": "vless",
        "server": host,
        "port": int(port) if port else 443,  # 默认端口为443
        "uuid": uuid,
...
def parse_md_link(link):
    try:
        ...
        # 定义正则表达式模式，匹配所需的协议链接
        pattern = r'(?:vless|vmess|trojan|hysteria2|hy2|ss):\/\/[^#\s]*(?:#[^\s]*)?'
...
def handle_links(new_links, resolve_name_conflicts):
    ...
    for new_link in new_links:
        try:
            ...
            print(f"处理链接时出错: {str(e)}, 链接: {new_link}")
            continue  # 继续处理下一个链接
...
def generate_clash_config(links,load_nodes):
    ...
    def resolve_name_conflicts(node):
        try:
            # 确保所有字符串值不包含 YAML 特殊标记
            if isinstance(node, dict):
                for key, value in node.items():
                    if isinstance(value, str):
                        node[key] = str(value).replace('!<str>', '').strip()
            
            name = str(node["name"])
            if filter_node_name(name):
        ...
        try:
            if link.startswith(("hysteria2://", "hy2://","trojan://", "ss://", "vless://", "vmess://")):
                ...
                try:
                    ...
                    print(f"处理URL时出错，已跳过: {str(e)}")
                    continue
        except Exception as e:
            print(f"处理链接时出错，已跳过: {str(e)}")
            continue
    ...
    if config["proxies"]:
        try:
            # 处理所有节点的密码字段，确保移除 YAML 特殊标记
            for proxy in config["proxies"]:
                if "password" in proxy:
                    proxy["password"] = str(proxy["password"]).replace('!<str>', '').strip()
            ...
            try:
                ...
                with open(f'{CONFIG_FILE}.json', "w", encoding="utf-8") as f:
                    json.dump(config, f, ensure_ascii=False, indent=2)
                print(f"使用安全模式保存配置文件成功")
            except Exception as e:
                print(f"安全模式保存配置文件也失败: {e}")

    else:
        print('没有节点数据更新')

def filter_node_name(node_name):
    if any(banned_word in node_name for banned_word in BAN):
        return False
    return any(allowed_word in node_name for allowed_word in ALLOWED)
...
class ClashConfig:
    ...
    def save(self):
        ...
        # print(f"读取更新后的配置 clash_config.yaml")
        # # 定义要读取的文件路径
        ...
        # print(f"读取更新后的配置 clash_config.yaml.json")
        # # 定义要读取的文件路径
        # file_path = "clash_config.yaml.json"
        # # 打开并读取文件内容
        # try:
        #     with open(file_path, "r", encoding="utf-8") as file:
        #         content = file.read()
...
async def proxy_clean():
    ...
    async with ClashAPI(CLASH_API_HOST, CLASH_API_PORTS, CLASH_API_SECRET) as clash_api:
        ...
        try:
            ...
            print('\n===================移除失效节点并按延迟排序======================\n')
            # TODO 只对proxy_groups进行了排序，没有对proxies进行排序
            # 一次性移除所有失效节点并更新配置
            ...
            if SPEED_TEST:
                ...
                # 调用函数并获取返回的两个列表
                sorted_proxy_names, sorted_proxy_names_speed = start_download_test(proxy_names)

                # 获取 sorted_proxy_names 列表
                print("Sorted Proxy Names:")
                print(sorted_proxy_names)

                # 获取 sorted_proxy_names_speed 列表
                print("Sorted Proxy Names with Speed:")
                print(sorted_proxy_names_speed)
                ...
                # 按测速结果排序好的节点名放入 proxy_groups
                for group_name in groups_to_test:
                    for group in config.proxy_groups:
                        if group["name"] == group_name:
                            group["proxies"] = sorted_proxy_names_speed
                print(f"proxy_groups 包含的节点")
                print(f"{sorted_proxy_names}")
                # 保存更新后的配置
                config.save()

                # 一次性移除所有测速结果不达标的节点
                # 打开和加载 clash_config.yaml 文件
                with open("clash_config.yaml", "r", encoding="utf-8") as file:
                    config = yaml.safe_load(file)

                # 过滤掉 name 不在 sorted_proxy_names 的代理
                filtered_proxies = [
                    proxy for proxy in config.get("proxies", [])
                    if proxy.get("name") in sorted_proxy_names
                ]

                # 更新配置
                config["proxies"] = filtered_proxies
                print("一次性移除所有测速结果不达标的节点")                

                for proxy in config.get("proxies", []): 
                    if 'name' in proxy: 
                        for name_speed in sorted_proxy_names_speed: 
                            # print("{name_speed}节点名称加入测速结果") 
                            if name_speed.startswith(proxy['name']): 
                                proxy['name'] = name_speed
                                print("{name_speed}节点名称已加入测速结果") 
                                print(name_speed) 
...
def work(links,check=False,allowed_types=[],only_check=False):
    # 获取当前代理设置并打印
    original_proxies = get_proxy_settings()
    # 清空代理（包括小写和大写格式）
    empty_proxies = {key: None for key in original_proxies}
    set_proxy_settings(empty_proxies)
        ...
        if check or only_check:
            ...
            try:
                ...
                if clash_process is not None:
                    clash_process.kill()
                # 恢复代理设置并打印
                set_proxy_settings(original_proxies)
                # Copy the file
                source_path = "~/ClashForge/clash_config.yaml"
                destination_path = "/mnt/d/OneDrive"
                try:
                    os.system(f"cp {source_path} {destination_path}")
                    print("File copied successfully.")
                    send_telegram_notification('国外优选任务执行完毕')
                except Exception as e:
...
def test_proxy_speed(proxy_name):
    ...
    total_length = 0
    # 测试下载时间（秒）
    test_duration = 10  # 逐块下载，直到达到10秒钟为止
