...
def generate_clash_config(links,load_nodes):
    ...
    if config["proxies"]:
        ...
        # 修复hysteria2格式错误
        # 修复Clash配置文件clash_config.yaml
        with open("clash_config.yaml", "r", encoding="utf-8") as file:
            config = yaml.safe_load(file)
        for proxy in config.get("proxies", []):
            if 'type' in proxy and proxy['type'] == "hysteria2":
                if 'obfs-password' not in proxy:
                    if 'obfs' in proxy:
        ...
        # 修复Clash配置文件clash_config.yaml.json
        with open('clash_config.yaml.json', 'r', encoding='utf-8') as file:
        ...
        with open('clash_config.yaml.json', 'w', encoding='utf-8') as file:
            json.dump(config, file, indent=4, ensure_ascii=False)

        print("修复hysteria2格式错误")
